...
function setButtonsToolTip() {
    ...
    // Init buttons
    setTippy(initScreenShareBtn, 'Ekran Paylaşımı', 'top');
    setTippy(initVideoMirrorBtn, 'Video Yansıtma', 'top');
    // Main buttons
    refreshMainButtonsToolTipPlacement();
    // Chat room buttons
    setTippy(msgerClose, 'Kapat', 'bottom');
    setTippy(msgerShowChatOnMsgDiv, 'Yeni mesaj alındığında sohbeti göster', 'bottom');
    setTippy(msgerSpeechMsgDiv, 'Gelen mesajları seslendir', 'bottom');
    setTippy(msgerTheme, 'Gece modu', 'bottom');
    setTippy(msgerClean, 'Mesajları temizle', 'bottom');
    setTippy(msgerSaveBtn, 'Mesajları kaydet', 'bottom');
    setTippy(msgerMaxBtn, 'Büyüt', 'bottom');
    setTippy(msgerMinBtn, 'Küçült', 'bottom');
    setTippy(msgerEmojiBtn, 'Emoji', 'top');
    setTippy(msgerMarkdownBtn, 'Markdown', 'top');
    setTippy(msgerGPTBtn, 'ChatGPT', 'top');
    setTippy(msgerShareFileBtn, 'Dosya paylaş', 'top');
    setTippy(msgerCPBtn, 'Özel mesajlar', 'top');
    setTippy(msgerCleanTextBtn, 'Temizle', 'top');
    setTippy(msgerPasteBtn, 'Yapıştır', 'top');
    setTippy(msgerSendBtn, 'Gönder', 'top');
// Sohbet katılımcıları düğmeleri
    setTippy(msgerCPCloseBtn, 'Kapat', 'bottom');
// Altyazı düğmeleri
    setTippy(captionClose, 'Kapat', 'bottom');
    setTippy(captionMaxBtn, 'Büyüt', 'bottom');
    setTippy(captionMinBtn, 'Küçült', 'bottom');
    setTippy(captionTheme, 'Gece modu', 'bottom');
    setTippy(captionClean, 'Mesajları temizle', 'bottom');
    setTippy(captionSaveBtn, 'Mesajları kaydet', 'bottom');
    setTippy(speechRecognitionIcon, 'Durum', 'bottom');
    setTippy(speechRecognitionStart, 'Başlat', 'top');
    setTippy(speechRecognitionStop, 'Durdur', 'top');
    ...
    setTippy(whiteboardImgFileBtn, 'Dosyadan resim ekle', 'bottom');
    setTippy(whiteboardPdfFileBtn, 'Dosyadan PDF ekle', 'bottom');
    setTippy(whiteboardImgUrlBtn, 'URL\'den resim ekle', 'bottom');
    ...
    setTippy(whiteboardSaveBtn, 'Tahtayı kaydet', 'bottom');
    setTippy(whiteboardEraserBtn, 'Nesneyi sil', 'bottom');
    setTippy(whiteboardCleanBtn, 'Tahtayı temizle', 'bottom');
    setTippy(whiteboardLockBtn, 'Etkinse, katılımcılar etkileşimde bulunamaz', 'sağ');
    setTippy(whiteboardCloseBtn, 'Kapat', 'sağ');
// Dosya transferi düğmeleri
    setTippy(sendAbortBtn, 'Dosya transferini iptal et', 'sağ başlangıç');
    setTippy(receiveAbortBtn, 'Dosya transferini iptal et', 'sağ başlangıç');
    setTippy(receiveHideBtn, 'Dosya transferini gizle', 'sağ başlangıç');
// Video/ses URL oynatıcı
    setTippy(videoUrlCloseBtn, 'Video oynatıcıyı kapat', 'bottom');
    setTippy(videoAudioCloseBtn, 'Video oynatıcıyı kapat', 'bottom');
    setTippy(msgerVideoUrlBtn, 'Tüm katılımcılara video veya ses paylaş', 'üst');
...
function refreshMainButtonsToolTipPlacement() {
    ...
    setTippy(shareRoomBtn, 'Toplantıyı paylaş', placement);
    setTippy(hideMeBtn, 'Kendimi odadan gizle/göster', placement);
    setTippy(audioBtn, useAudio ? 'Sesi durdur' : 'Sesim kapalı', placement);
    setTippy(videoBtn, useVideo ? 'Görüntüyü durdur' : 'Görüntüm kapalı', placement);
    setTippy(screenShareBtn, 'Ekran paylaşımı başlat', placement);
    setTippy(recordStreamBtn, 'Kaydı başlat', placement);
    setTippy(fullScreenBtn, 'Tam ekran görüntüle', placement);
    setTippy(chatRoomBtn, 'Sohbeti aç', placement);
    setTippy(captionBtn, 'Altyazıyı aç', placement);
    setTippy(roomEmojiPickerBtn, 'Reaksiyon gönder', placement);
    setTippy(myHandBtn, 'Elinizi kaldırın', placement);
    setTippy(whiteboardBtn, 'Beyaz tahtayı aç', placement);
    setTippy(fileShareBtn, 'Dosya paylaş', placement);
    setTippy(documentPiPBtn, 'Resim içinde resmi aç/kapat', placement);
    setTippy(mySettingsBtn, 'Ayarları aç', placement);
    setTippy(aboutBtn, 'Bu proje hakkında', placement);
    setTippy(leaveRoomBtn, 'Odadan ayrıl', placement);
;
}
...
async function whoAreYou() {
    ...
    Swal.fire({
        ...
        confirmButtonText: `Toplantıya katılın`,
        customClass: { popup: 'init-modal-size' },
        showClass: { popup: 'animate__animated animate__fadeInDown' },
        hideClass: { popup: 'animate__animated animate__fadeOutUp' },
        inputValidator: async (value) => {
            if (!value) return 'Lütfen adınızı girin';
            // Long name
            if (value.length > 30) return '30 karakterle sınırlıdır';
    ...
    setTippy(initAudioBtn, 'Sesi kapat', 'top');
    setTippy(initVideoBtn, 'Videoyu Kapat', 'top');
}
...
function shareRoomMeetingURL(checkScreen = false) {
    ...
    Swal.fire({
        ...
        <br/>
        <p style="color:rgb(8, 189, 89);">Toplantıyı mobil veya aygıt için paylaş</p>
        <p style="background:transparent; color:white; font-family: Arial, Helvetica, sans-serif;">Telefonunuzla QR kodunu tarayın veya başka birini davet etmek için aşağıdaki URL'yi gönderin.</p>
...
function copyRoomURL() {
    ...
    userLog('toast', 'Toplantı linki kopyalandı 👍');
}
...
function showAbout() {
    ...
    Swal.fire({
        background: swBg,
        position: 'center',
        title: '<strong>HawkMeet v1.3.0</strong>',
        ...
        <div id="about">
            <button 
                ...
                <i class="${className.heart}" ></i>&nbsp;Destek?
            </button>
            <br /><br /><br />
            Geliştirici:<a 
                id="linkedin-button" 
                data-umami-event="Linkedin button" 
                href="https://linkedin.com/in/mehmet-furkan-öğütcü-622139178/" target="_blank"> 
                Mehmet Furkan Öğütcü
            </a>
            <br /><br />
            Email:<a 
                id="email-button" 
                data-umami-event="Email button" 
                href="mailto:furkan@ogutcu.com.tr?subject=Hawkmeet info"> 
                furkan@ogutcu.com.tr
            </a>
            <br /><br />
            <hr />
            <span>&copy; 2024 HawkMeet, all rights reserved</span>
            <hr />
        </div>
        `,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
